function complex_hiv_model_def
    clear; clc; close all;  
    ...
    figure('Color', 'w', "Name", "Complex Model Dynamics");
...
function dydt = ode_system(t, y)
    ...
    kv_t = kv_bar * (1 + t/t3) * u_t; % Time-varying Virus infection rate
    e_bar = 0.064;  % Baseline Rate of infection of thymic T4 cells
    e_t = e_bar * (1 + t/t3) * u_t; % Time-varying Thymic infection rate
    kvm_bar = 1.19; % Baseline Rate of infection of healthy macrophages
    kvm_t = kvm_bar * (1 + t/t3); % Time-varying Macrophage infection rate
